...
def FGvsSG_xErrL2L2_test1_singular_type1(dir_fig, stabParamsType, showPlot, savePlot, plotId):
    ndim = 2
    ...
    if plotId == 1:
        ...
        myPlotDict['outFileName'] = dir_fig + "FGvsSG_test1_singular_type1_deg1_v.eps"
        myPlotDict['ylim'] = [2e-4, 2]
        ...
        myPlotDict['outFileName'] = dir_fig + "FGvsSG_test1_singular_type1_deg1_sigma.eps"
        myPlotDict['ylim'] = [2e-4, 2]
        plotData(data_x, data_y, ref_data_y, myPlotDict)
        ...
        myPlotDict['outFileName'] = dir_fig + "FGvsSG_test1_singular_type1_deg1.eps"
        myPlotDict['ylim'] = [2e-4, 2]
        plotData(data_x, data_y, ref_data_y, myPlotDict)
    ...
    if plotId == 2:
        ...
        myPlotDict['ylabel'] = r'Rel. error $\left|| v - \hat{v}_h \right||_{L^2(\Omega\times\{T\})}$ [log]'
        myPlotDict['outFileName'] = dir_fig + "FGvsSG_test1_singular_type1_deg2_v.eps"
        myPlotDict['ylim'] = [6e-6, 2]
        plotData(data_x, data_y, ref_data_y, myPlotDict)
        ...
        myPlotDict[
            'ylabel'] = r'Rel. error $\left|| \mathbf{\sigma} - \hat{\mathbf{\sigma_h}} \right||' \
                        r'_{L^2(\Omega\times\{T\})^{%d}}$ [log]' % ndim
        myPlotDict['outFileName'] = dir_fig + "FGvsSG_test1_singular_type1_deg2_sigma.eps"
        myPlotDict['ylim'] = [6e-6, 2]
        plotData(data_x, data_y, ref_data_y, myPlotDict)
        ...
        myPlotDict['ref_data_labels'] = ['$O(M_L^{-1.5})$', '$O(M_L^{-1})$']
        myPlotDict['ylabel'] = r'Total error $\mathcal{E}$ [log]'
        myPlotDict['outFileName'] = dir_fig + "FGvsSG_test1_singular_type1_deg2.eps"
        myPlotDict['ylim'] = [6e-6, 2]
        plotData(data_x, data_y, ref_data_y, myPlotDict)
...
if __name__ == '__main__':
    ...
    plotId_ = 2
    stabParamsType_ = 4
    FGvsSG_xErrL2L2_test1_singular_type1(dir_fig_, stabParamsType_, showPlot_, savePlot_, plotId_)
